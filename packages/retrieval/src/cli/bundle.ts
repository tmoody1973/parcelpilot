...
// Usage: pnpm evidence:bundle --taxkey 2500011000 --scenario G01 [--budget 6000] [--out path]
...
const gold = JSON.parse(readFileSync(join(root, "packages", "contracts", "gold", `${scenarioId}.json`), "utf8")) as { scenario: { use: string; ground_floor_use?: string } };
...
try {
  ...
  const use = gold.scenario.use === "multifamily" ? "multi-family dwelling" : gold.scenario.use;
  const questions: Array<[string | null, string]> = [
    ["use", `Is a ${use} allowed in ${d}, with ${gold.scenario.ground_floor_use ?? "no"} on the ground floor?`],
    ["height", `maximum and minimum building height in ${d}`],
  ...
  const bundle = await assembleBundle(sql, { retrievalRunIds: runIds, tokenBudget: Number(flag("--budget") ?? 6000), analysisDate });
