...
**Decision.** When a reviewer approves a rule candidate whose criticality is `critical` (the rules most likely to decide a feasibility call: use, height, density), no rule is created. The candidate stays in review with that reviewer recorded as the first sign-off, and only an approval by a member with the `owner` role mints the rule. An owner's approval mints directly, with or without a prior reviewer sign-off. Every other criticality mints on the first approval by any reviewer. Two guards keep the bar where the extraction set it: a reviewer may raise a candidate's criticality but only an owner may lower it, and the approval gate reads the more severe of the live proposal and the queue task's priority, which was set when the task was generated and is never edited. (Decision 010 is reserved for the footnote policy in MOO-825.)
