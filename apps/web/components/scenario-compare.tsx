...
const ROWS: { label: string; get: (s: Scenario) => string }[] = [
  { label: "Ground-floor use", get: (s) => s.use ?? "—" },
  { label: "Units", get: (s) => (s.units ?? "—").toString() },
  { label: "Stories", get: (s) => (s.stories ?? "—").toString() },
  { label: "Height (ft)", get: (s) => (s.height_ft ?? "—").toString() },
  { label: "Parking spaces", get: (s) => (s.parking_spaces ?? "—").toString() },
  { label: "Ground-floor commercial (sq ft)", get: (s) => (s.ground_floor_commercial_sqft ?? "—").toString() },
];
