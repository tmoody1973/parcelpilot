...
import { computeIntersections, createParcelStore, intersectingFeatures } from "@parcelpilot/db";
...
export async function resolveSite(input: ResolveInput): Promise<ResolveResult> {
  ...
  const { summary } = await computeIntersections(sql, result.snapshot_id);
  const [rows, features] = await Promise.all([
    ...
    intersectingFeatures(sql, result.snapshot_id),
  ]);
  return { kind: "resolved", profile: assembleProfile(result.facts, result.snapshot_id, result.snapshot_reused, summary, rows[0]?.retrieved_at ?? null, features) };
}

// Rebuilds the profile from the parcel's latest stored snapshot without a fresh ArcGIS call (project history).
// The stored attributes are the original ArcGIS properties, so toFacts reproduces the same field mapping.
export async function profileForTaxkey(taxkey: string): Promise<SiteProfile | null> {
  const sql = serviceSql();
  const rows = await sql<{ id: string; attributes: Record<string, unknown>; retrieved_at: string; geometry: GeoJsonPolygon }[]>`
    select id, attributes, retrieved_at, ST_AsGeoJSON(geometry)::json as geometry from parcel_snapshots where taxkey = ${taxkey} order by retrieved_at desc limit 1`;
  const row = rows[0];
  if (!row) return null;
  const { summary } = await computeIntersections(sql, row.id);
  const features = await intersectingFeatures(sql, row.id);
  const facts = toFacts({ type: "Feature", geometry: row.geometry, properties: row.attributes } satisfies ParcelFeature);
  return assembleProfile(facts, row.id, true, summary, row.retrieved_at, features);
}

function assembleProfile(facts: ParcelFacts, snapshotId: string, reused: boolean, summary: GisSummary, retrievedAt: string | null, features: SiteProfile["gis_features"]): SiteProfile {
  return {
    taxkey: facts.taxkey,
