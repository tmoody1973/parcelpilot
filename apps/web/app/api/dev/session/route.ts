...
// GET /api/dev/session?user=alice&org=alpha&orgName=Alpha%20Dev&role=reviewer   (404 when dev auth mode is off)
export async function GET(req: Request): Promise<Response> {
  ...
  const role = u.searchParams.get("role") ?? "member";
  const res = NextResponse.redirect(new URL("/", u.origin));
  for (const [k, v] of [["pp-dev-user", user], ["pp-dev-org", org], ["pp-dev-org-name", orgName], ["pp-dev-role", role]] as const) res.cookies.set(k, v, { path: "/", httpOnly: true, sameSite: "lax" });
  return res;
}
